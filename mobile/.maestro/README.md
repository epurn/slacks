...
| `resolve.yaml` | Entry-resolve beat (beat 1) real data-path + no-layout-shift guard: submit a multi-item log → one pending skeleton row appears → Refresh loads the completed item-forward `/log-events/by-date` feed → one resolved summary row ("Greek yogurt and 1 more item, 245 kcal total") renders from real derived-item data, no second item row appears, and the hero counts the entry (a data-starved or row-growing Today fails here) (FTY-180/181) |

...
| `adherence-strip-fty431.yaml` | Trends intake-adherence strip: open `trends.adherence_mix` (a range fixture whose newest days cover on-target, off-target, no-target and no-data) in dark and light, assert each cell state is present through its accessibility label, and screenshot the strip — the running-app proof that the off-target cell lost its `surface`-colored ring (a near-black outline in dark mode) and still reads apart from on-target by silhouette, not hue (FTY-431) |

...
| `tab-bar-occlusion.yaml` | Tab-bar occlusion idiom: seed a timeline taller than the screen → pull-to-refresh loads the multi-item feed → scroll rows *under* the floating, blurred `.ultraThin` tab bar → the tab labels and the app-drawn dimming `TabBarScrim` (testID `tab-bar-scrim`) stay pinned over the scrolled content, and the last row is reachable only after scrolling — content passes under the bar, not fully legible through it (FTY-185) |

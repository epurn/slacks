 ...
 *
 * This is an in-memory, app-session-scoped value — **not persisted to disk**
 * and not fetched from the server (FTY-189 is explicitly no-new-persistence) —
 * so it reflects the most recent goal create/edit made *this session*. It
 * resets to unknown on sign-out/sign-in so a new account never inherits a stale
 * direction. Screens that read it (Trends) treat "unknown" as **neutral** — no
 * toward/away claim rather than a guessed default — so a returning gain/maintain
 * user is never mis-colored; screens that know it (Settings, Onboarding) call
 * `setGoalDirection` right after a successful goal save.
 */

...
from app.estimator.correction_resolution import PRIOR_CORRECTION_SOURCE
...
if TYPE_CHECKING:
    # Type-only import for the delegating method's return annotation; the runtime
    # import is deferred into the method body to keep the re_match <->
    # prior_correction_candidates edge off the module-load path (FTY-413).
...
def _evidence_row(session: Session, item: DerivedFoodItem) -> EvidenceSource:
    ...
    re-matched item keeps exactly one evidence row now pointing at the new source. A
    shared cross-module helper: :meth:`ReMatchCapability._rewrite_evidence` (source-cache
    rewrite) and ``prior_correction_candidates._rewrite_prior_correction_evidence``
    (FTY-413 extraction) both look up the row through it, so the write paths cannot drift.
    """
...
class ReMatchCapability:
    ...
    def list_prior_correction_candidates(
        ...
        *,
        ...
        Owns the per-user fail-closed authorization + object-scoped item load, then
        delegates the candidate projection to
        :func:`app.estimator.prior_correction_candidates.list_prior_correction_candidates`
        (FTY-413 extraction) — a cross-user or unknown item fails closed here (as in
        :meth:`list_alternatives`) before any correction row is read, so another user's
        correction is never surfaced. Reads only — no network egress, no cache write.
        """

        self._authorize(owner_id, current_user)
        item = self._load_owned(item_id, owner_id)

        # Deferred import: prior_correction_candidates imports the shared re_match
        # audit/evidence helpers, so importing it at call time keeps the two-way edge
        # off the module-load path (no re_match <-> prior_correction_candidates cycle).
        from app.estimator import prior_correction_candidates  # noqa: PLC0415 — import cycle

        return prior_correction_candidates.list_prior_correction_candidates(self.session, item)
    ...
    def re_resolve(
        ...
        *,
        ...
        # The apply path lives in prior_correction_candidates (FTY-413); import it at
        # call time so the shared-helper edge stays off the module-load path.
        if source_ref.startswith(f"{PRIOR_CORRECTION_SOURCE}:"):
            from app.estimator import prior_correction_candidates  # noqa: PLC0415 — import cycle

            return prior_correction_candidates.apply_prior_correction(
                self.session, item, source_ref
            )
    ...
    def _rewrite_evidence(
        ...
        evidence = _evidence_row(self.session, item)

...
| Transient provider failure | `StepError` (retryable) | rough estimate on degrade (`food-resolution-changelog.md` v23) | retries within bound, then **degrades** (FTY-370, `estimation-jobs.md` v7): `processing → completed` / `partially_resolved` with ≥1 interpreted candidate, else honest still-working `processing`; never terminal `failed` |

...
| `FATTY_ESTIMATOR_MAX_PARSE_REPAIR_ATTEMPTS` | `2` | `0`-`10` | pre-`ParseResult` provider-output validation | Maximum deterministic recovery passes for explicitly enumerated provider schema-shape mistakes before a `ParseResult` exists; it does not repair schema-valid policy conflicts or provider clarification decisions. |

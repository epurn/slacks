...
| `FATTY_ESTIMATOR_MAX_PARSE_REPAIR_ATTEMPTS` | `2` | `0`-`10` | all modes; pre-`ParseResult` provider-output validation | Maximum deterministic recovery passes for explicitly enumerated provider schema-shape mistakes before a `ParseResult` exists, and for bounded retry/repair attempts when schema-valid provider output conflicts with the active policy, such as returning a clarification for a recoverable identity. |

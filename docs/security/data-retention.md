...
- Estimation runs: store model/provider, schema version, tool names, source references, assumptions, validation errors, and sanitized traces. The trace's structured decision entries (FTY-255, `docs/contracts/estimation-jobs.md` "Decision trace") hold bounded sanitized labels, clamped counts, and non-secret source references only — an embedded URL keeps scheme/host/path with the query string, fragment, and userinfo dropped; labels are control-character-stripped and redacted of secret-looking material; the entry count is capped per run. No raw event text, prompts, provider output, fetched pages, search snippets, or keys/tokens are ever stored; a global source row's bounded description may appear (global source data, not user data). The recorded provider/model are the configured selector and model string — operator configuration, not secrets. Runs remain user-tied rows and follow the owning log event's retention (`ON DELETE CASCADE`).

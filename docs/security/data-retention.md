...
- Evidence sources (`evidence_sources`): user-owned provenance for a resolved food item (FTY-044 generic foods; FTY-060 barcode/Open Food Facts; FTY-061 user labels; FTY-062 official-source pages and model-prior estimates) — the source reference (e.g. `usda_fdc:<fdcId>`, `open_food_facts:<barcode>`, `official_source:<url>`, or `model_prior`), content hash, fetch timestamp, and an extracted per-100g facts snapshot. For a model-prior fallback (FTY-062) a nullable `assumptions` JSON column records why the fallback was used (and any density/serving assumptions), so the entry surfaces an explicit source status and stays user-editable; it holds no raw user text. An official-source record stores the page **URL** only — never the raw fetched page — and no raw provider response is stored. Retained until the owning log event, user, or account is deleted (`ON DELETE CASCADE`).

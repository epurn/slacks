...
- Cached search lookups (search-call hygiene, FTY-435): **in process memory only, not
  stored.** To stop the estimator re-issuing the same query to the search provider, an
  *answered* lookup (`success` / `partial`) is held in a small in-process cache keyed by
  the provider key plus the **sanitized** query — the exact string that already egresses
  (`sanitize_query`: item identity only, control characters stripped, length-bounded) —
  with the provider's candidate result URLs, bounded titles, and bounded snippets as the
  value. Like `products` it is **global source data with nothing user-specific**: no user
  id, no raw diary text, no profile/body/goal context, no credential, and no
  failure/rate-limit outcome. It lives only in the worker/API process (never Postgres,
  never Redis, never a log line, never a trace), expires on a short bounded TTL
  (`SLACKS_SEARCH_CACHE_TTL_SECONDS`, default 300 s) and a bounded entry count, and dies
  with the process — so it adds **no** new persisted retention surface and there is
  nothing to delete on user/account deletion. The same module's rate-limit cooldown holds
  a single expiry timestamp per provider, no query data at all. Its telemetry is counts
  and closed-vocabulary statuses; query text is never logged
  (`docs/contracts/evidence-retrieval.md`, **Search-call hygiene — FTY-435**).

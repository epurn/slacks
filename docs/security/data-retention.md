...
- Evidence sources (`evidence_sources`): user-owned provenance for a resolved food item (FTY-044) — the source reference, content hash, fetch timestamp, and an extracted per-100g facts snapshot. Never a raw fetched page. Retained until the owning log event, user, or account is deleted (`ON DELETE CASCADE`).
- Cached source facts (`products`): global trusted-source nutrition facts (USDA FDC) with **no** user-specific data — the per-100g facts for a generic food are the same for everyone. Cached to avoid repeat external lookups and retained as global source facts (nothing user-specific to delete); the `evidence_sources` link to a product is `ON DELETE SET NULL`, so clearing the cache never removes user-owned evidence.

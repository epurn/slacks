...
| `fty433-2-save-on-light.png` / `-dark.png` | One tap on the save control makes the sticky opt-in unmistakable: amber-filled glyph, amber hairline, and a persistent "Saving photos" caption. (The flow then turns it back off, so the logging run below is the steady-state, discard-by-default one.) |
